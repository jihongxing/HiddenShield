...
- 发布 Windows x64 NSIS 安装器 `HiddenShield_0.1.3_x64-setup.exe` 与 MSI 安装器 `HiddenShield_0.1.3_x64_en-US.msi`。
- GitHub Release 同时发布 Tauri updater 签名、`latest.json`、`SHA256SUMS.txt` 与 JSON 发布清单。
- 发布资产、SHA-256、签名口径和下载入口见 `docs/桌面v0.1.3发布清单.md`。

### 产品与文案

- 对外 README 聚焦 Windows 桌面图片 / 音频保护与技术校验，不再把视频、真实支付、公共信任层或移动端冻结资产写成当前发布能力。
- 统一“载荷认证标签匹配”“技术校验”“版权编号生成”“完整性匹配”和法律效力边界，避免将其表述为发行方数字签名、实名认证、权属确认或司法证明。
- 感知质量实验室继续作为独立内部 Windows 工具；其 ABX 结果只适用于当前素材、设备和测试环境，不构成“绝对无感”承诺。

### 已知限制

- GitHub Actions 发布的公开安装器采用 Tauri Ed25519 updater 签名，不提供公开 Authenticode 签名主体。
- 自动更新仍属于内部验证范围；当前推荐手动下载并校验 `SHA256SUMS.txt` 后安装。
- `v0.1.2` 仍为 Draft Release，当前没有可对用户承诺的公开回滚安装包。
- 真实支付、公共信任层、移动端新增能力与视频画面盲水印不包含在本次发布范围。
