...
// ---------------------------------------------------------------------------
// Helper: ensure FFmpeg paths are available (detect or download)
// ---------------------------------------------------------------------------
...
async fn ensure_ffmpeg_paths(app_handle: &AppHandle) -> Result<ffmpeg::FfmpegPaths, String> {
  ...
  // Try detection first
  match ffmpeg::detect_ffmpeg(&app_data_dir).await {
    Ok(paths) => {
      let _ = state.ffmpeg_paths.set(paths.clone());
      return Ok(paths);
    }
    Err(_) => {
      // Detection failed — attempt download
    }
  }

  // Download FFmpeg
  let paths = ffmpeg::download_ffmpeg(&app_data_dir, |_downloaded, _total| {})
    .await
    .map_err(|e| format!("FFmpeg 下载失败: {e}"))?;
...
async fn ensure_hw_info(
  ...
  if let Some(info) = state.hw_info.get() {
    return info.clone();
  }

  let detected = hw_detect::detect_hardware(&ffmpeg_paths.ffmpeg).await;
  let _ = state.hw_info.set(detected.clone());
  detected
}
...
pub async fn start_pipeline(
  ...
  {
    let mut active = state.active_pipelines.lock().map_err(|err| err.to_string())?;
    active.insert(pipeline_id.clone());
  }

  // Resolve FFmpeg paths and hardware info before spawning
  let ffmpeg_paths = ensure_ffmpeg_paths(&app_handle).await?;
  let hw_info = ensure_hw_info(&app_handle, &ffmpeg_paths).await;

  let summary = if is_media_only {
    let type_label = if file_type == scheduler::FileType::Image { "图片" } else { "音频" };
    format!("已创建{}水印嵌入任务", type_label)
  } else {
    format!(
      "已为 {} 个平台创建压制任务（{}）",
      platforms.len(),
      hw_info.preferred_encoder,
    )
  };
  ...
  Ok(PipelineStartResult { pipeline_id, summary })
}
...
#[tauri::command]
pub async fn check_active_pipelines(
  state: State<'_, AppState>,
) -> Result<Vec<String>, String> {
  let active = state.active_pipelines.lock().map_err(|err| err.to_string())?;
