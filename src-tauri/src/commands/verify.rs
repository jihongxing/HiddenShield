...
pub struct VerificationResult {
  ...
  /// TSA attestation info (if available from the matched record)
...
pub async fn verify_suspect(
  ...
  if let Some(ref payload) = payload {
    ...
    // Look up in DB using UID + file_hash prefix for precise matching
    let matched_record = {
    ...
    if confidence >= 0.95 {
      ...
      let tsa_verified = matched_record.as_ref()
...
async fn extract_from_audio_bearing(
  ...
  let ffmpeg_paths = {
    if let Some(paths) = state.ffmpeg_paths.get() {
      ...
      ffmpeg::detect_ffmpeg(&app_data_dir)
        .await
  ...
  let payload = watermark::extract_watermark(&samples)
    .map_err(|e| format!("水印提取失败: {e}"))?;
...
fn read_wav_samples(wav_path: &Path) -> Result<Vec<f32>, String> {
  let mut reader =
    hound::WavReader::open(wav_path).map_err(|e| format!("打开 WAV 失败: {e}"))?;
