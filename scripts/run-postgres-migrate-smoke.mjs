...
const dbName = 'hiddenshield_migrate_smoke';
const externalUrl = process.env.HIDDENSHIELD_POSTGRES_TEST_DATABASE_URL || process.env.DATABASE_URL;
const runId = `postgres-migrate-smoke-${Date.now()}`;
const artifactDir = resolve('tmp-ui-qa/postgres-migration');
...
if (!containerRuntime) {
  console.error(
    'cloud:postgres-migrate-smoke requires either HIDDENSHIELD_POSTGRES_TEST_DATABASE_URL or Podman/Docker.',
  );
  console.error(
    'Use a disposable database URL containing localhost/127.0.0.1 and hiddenshield_migrate_smoke.',
  );
  process.exit(2);
}

const containerName = `hiddenshield-postgres-smoke-${Date.now()}-${randomInt(1000, 9999)}`;
...
async function waitForPostgresAndRunSmoke(url) {
  ...
  while (Date.now() < deadline) {
    ...
    if (!lastOutput.includes('error communicating with database') && !lastOutput.includes('Connection refused')) {
      throw new Error(lastOutput || 'postgres migrate smoke failed before readiness');
    }
    await sleep(1500);
  }
...
function detectContainerRuntime() {
  for (const candidate of ['podman', 'docker']) {
    ...
    if (result.status === 0) {
      console.log(`Using ${candidate} for disposable Postgres smoke.`);
      return {
        kind: candidate,
...
function parseSmokeJson(stdout) {
  const line = stdout
    ...
    .at(-1);
  if (!line) {
    throw new Error(`postgres migrate smoke did not emit JSON: ${stdout}`);
  }
...
function writeArtifact({ runId, runtime, image, containerName, port, databaseName, smoke, cleanup }) {
  if (smoke.rollback !== 'empty_schema_verified') {
    throw new Error(`postgres migrate smoke rollback check failed: ${smoke.rollback}`);
  }
  mkdirSync(artifactDir, { recursive: true });
  const artifact = {
    schemaVersion: 'postgres_migration_smoke_artifact_v1',
    runId,
    ...
    databaseName,
    checks: {
      upTablesChecked: smoke.upTablesChecked,
      indexesChecked: smoke.indexesChecked,
      rollback: smoke.rollback,
    },
    safety: {
      requiresDisposableDatabaseName: 'hiddenshield_migrate_smoke',
  ...
  console.log(`Postgres migration smoke artifact: ${path}`);
}
