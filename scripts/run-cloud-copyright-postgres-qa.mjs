...
const databaseName = "hiddenshield_migrate_smoke_cloud_copyright_qa";
const externalUrl =
  process.env.HIDDENSHIELD_POSTGRES_TEST_DATABASE_URL || process.env.DATABASE_URL;
const runner = "cloud_copyright_postgres_concurrency_qa";
...
try {
  if (!databaseUrl) {
    ...
    if (!runtime) {
      throw new Error(
        "cloud:copyright-postgres-qa requires HIDDENSHIELD_POSTGRES_TEST_DATABASE_URL or Podman/Docker.",
      );
    }
    const port = String(randomInt(45433, 55432));
    containerName = `hiddenshield-cloud-copyright-qa-${Date.now()}-${randomInt(1000, 9999)}`;
  ...
  console.log(
    JSON.stringify({
      ok: true,
      schemaVersion: "cloud-copyright-postgres-concurrency-qa-v1",
      runner,
      scenarios: 8,
