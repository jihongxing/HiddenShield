...
const dbName = 'hiddenshield_sync_runtime_qa';
const externalUrl = process.env.HIDDENSHIELD_POSTGRES_TEST_DATABASE_URL || process.env.DATABASE_URL;
const runId = `cloud-sync-postgres-runtime-qa-${Date.now()}`;
const artifactDir = resolve('tmp-ui-qa/postgres-sync-runtime');
...
if (!containerRuntime) {
  console.error(
    'cloud:sync-postgres-runtime-qa requires either HIDDENSHIELD_POSTGRES_TEST_DATABASE_URL or Podman/Docker.',
  );
  console.error(
    'Use a disposable database URL containing localhost/127.0.0.1 and hiddenshield_sync_runtime_qa.',
  );
  process.exit(2);
}

const containerName = `hiddenshield-sync-postgres-qa-${Date.now()}-${randomInt(1000, 9999)}`;
  ...
  while (Date.now() < deadline) {
    const result = spawnSync(
      ...
      [
        ...
        'postgres',
        '--bin',
        'cloud_sync_postgres_runtime_qa',
      ],
      {
        env: {
          ...process.env,
          HIDDENSHIELD_POSTGRES_TEST_DATABASE_URL: url,
          HIDDENSHIELD_SYNC_POSTGRES_QA_RUN_ID: runId,
        },
        encoding: 'utf8',
    ...
    if (
      ...
      throw new Error(lastOutput || 'cloud sync Postgres runtime QA failed before readiness');
    }
    await sleep(1500);
  }
...
function detectContainerRuntime() {
  for (const candidate of ['podman', 'docker']) {
    ...
    if (result.status === 0) {
      console.log(`Using ${candidate} for cloud sync Postgres runtime QA.`);
      return {
        kind: candidate,
...
function parseQaJson(stdout) {
  const line = stdout
    ...
    .at(-1);
  if (!line) {
    throw new Error(`cloud sync Postgres runtime QA did not emit JSON: ${stdout}`);
  }
...
function writeArtifact({ runId, runtime, image, containerName, port, databaseName, qa, cleanup }) {
  if (qa?.ok !== true) {
    throw new Error(`cloud sync Postgres runtime QA failed: ${JSON.stringify(qa)}`);
  }
  if (qa?.checks?.registryRepositoryWritePath !== 'not_executed') {
    throw new Error('cloud sync Postgres runtime QA must not execute registry repository writes');
  }
  ...
  if (artifact.safety.productionDatabaseAllowed) {
    throw new Error('cloud sync Postgres runtime QA artifact must never allow production database');
  }
  const path = resolve(artifactDir, `${runId}.json`);
  writeFileSync(path, `${JSON.stringify(artifact, null, 2)}\n`, 'utf8');
  console.log(`Cloud sync Postgres runtime QA artifact: ${path}`);
}
