...
4. 使用 `node scripts/verify-ai-transparency-external-readiness.mjs <manifest> --mode review` 执行真实引用的 preflight；模板使用 `--mode template`。
...
- `template` 模式只接受带占位符的 `configuration_required` 模板；`review` 模式只接受无占位符、完整引用的 `ready_for_internal_review` manifest。
- review preflight 成功仅表示引用形状可评审，不表示 provider 可用、Sandbox 已验收、法规适用、生产 credential 已发放或 SDK 可对外发布。
