...
- 状态：`c0_contract_frozen_no_watermark_core_change`。
- 云版权库只能同步 `watermark-core` 已产生的 watermark UID、revision、parent UID、媒体/证据摘要、写后回读与验证状态；不得实现第二套 payload 编码、编号生成、写入、提取或重写判断。
- `cloud-copyright-record-v1` fixture 必须由同一组 `watermark-core` 结果生成，并纳入 desktop↔mobile 双向读取 Gate；云端 event 投影或公开 Registry 不能替代媒体本体读取。
- 原始媒体、保护副本和本地路径默认不上云，故云同步成功不构成跨端水印读取已验证。
- C0 已完成：云版权库 fixture 固定同步 watermark UID、revision、parent UID 和验证摘要，没有新增或修改 `watermark-core` 算法、payload 或接口。
- 下一共享核心任务：C1 迁移测试必须证明云端投影不生成或改写 watermark UID/revision；媒体本体跨端读取仍由既有 `watermark-core` fixture Gate 负责。
...
## 2026-08-01 苹果实况照片复合媒体核心约束

- Live Photo 是静态照片、配对视频和关联元数据构成的复合资产，不得以 Live Photo 名义只调用图片算法。
- 未来静态资源、视频画面和适用的视频音轨盲水印必须全部由 `watermark-core` 提供；平台层只负责资源配对、容器、元数据、事务和展示。
- 一个逻辑 Live Photo 可包含多个媒体级写入与读取结果，但必须共享同一个逻辑版权资产标识，并保留组件级摘要和验证状态。
- 正式发布必须覆盖静态写入/视频读取失败、视频写入/静态读取失败、配对视频替换、静态照片替换、音轨移除和关联元数据丢失等 fail-closed fixture。
- 当前不新增算法、payload 或 fixture；能力保持 `明确不能承诺`，直到视频画面生产 Gate 和 `docs/苹果实况照片复合媒体保护产品任务.md` 的发布 Gate 全部通过。

下一共享核心任务：

- 不提前实现 Live Photo 专用算法；先完成通用视频画面水印生产 Gate，再冻结复合资产共享标识和组件验证 envelope。
