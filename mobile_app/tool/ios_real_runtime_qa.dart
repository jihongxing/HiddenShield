...
Future<void> main() async {
  ...
  runApp(const _IosRuntimeQaApp());
}

class _IosRuntimeQaApp extends StatefulWidget {
  const _IosRuntimeQaApp();

  @override
  State<_IosRuntimeQaApp> createState() => _IosRuntimeQaAppState();
}

class _IosRuntimeQaAppState extends State<_IosRuntimeQaApp> {
  _IosRuntimeQaResult? _result;
  Object? _error;
  ...
  Future<void> _run() async {
    try {
      final result = await _runIosRuntimeQa();
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: 'HiddenShield iOS Runtime QA',
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('iOS QA 失败', style: Theme.of(context).textTheme.headlineSmall),
          const SizedBox(height: 12),
          Text('$error'),
        ],
      ),
...
class _ResultView extends StatelessWidget {
  ...
  final _IosRuntimeQaResult result;
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        Text(
          '证据来自 iOS 原生 Flutter 运行态、真实 Rust watermark bridge、真实 feedback-backend reserve / confirm / reconcile API。',
...
Future<_IosRuntimeQaResult> _runIosRuntimeQa() async {
  ...
  await appState.completeOnboarding(
    accountLabel: 'ios-real-runtime-$runId@hiddenshield.local',
    password: password,
    creatorLabel: 'iOS QA 创作者',
  );
  await _upgradeToCreator(appState, cloudClient, password: password);

  final cases = <_IosRuntimeQaCase>[];
  ...
  return _IosRuntimeQaResult(runId: runId, cases: cases);
}
...
Future<_IosRuntimeQaCase> _runKindCase(
  String runId,
  ...
  final onlineRecord = appState.addWriteResult(
    result: onlineResult,
    fileName: 'ios-${kind.name}-server-$runId',
    allowRewrite: false,
  ...
  final offlineRecord = appState.addWriteResult(
    ...
    fileName: 'ios-${kind.name}-offline-$runId',
    allowRewrite: false,
    registryResult: null,
  );
  final pendingRow = _IosRuntimeQaRow.fromRecord(
    kind: kind,
  ...
  return _IosRuntimeQaCase(
    kind: kind,
    rows: [
      _IosRuntimeQaRow.fromRecord(
        kind: kind,
...
class _IosRuntimeQaResult {
  const _IosRuntimeQaResult({required this.runId, required this.cases});

  final String runId;
  final List<_IosRuntimeQaCase> cases;
}

class _IosRuntimeQaCase {
  const _IosRuntimeQaCase({required this.kind, required this.rows});

  final WatermarkAssetKind kind;
  final List<_IosRuntimeQaRow> rows;
}

class _IosRuntimeQaRow {
  const _IosRuntimeQaRow({
