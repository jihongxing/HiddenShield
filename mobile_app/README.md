...
- 支持图片和 30 秒以上音频的正式写入 / 验证；发布门禁覆盖 WAV / MP3 / FLAC / OGG / M4A 的跨端互验。
- 本地不做视频盲水印。
- Rust / FRB 桥接层已接入同一 `watermark-core`，Web preview 不能作为正式能力证据。
- UI 与桌面端保持同一套工作台、取证、版权库、设置结构。
- 图片嵌入测试页已接入文件选择、预览、重写开关和 preview 结果摘要。
- 音频嵌入页已接入常见音频文件选择、30 秒时长门槛、重写开关和结果摘要。
- 取证页已接入图片 / 音频切换、文件选择和提取结果摘要。
...
- `PreviewWatermarkBridge` 仅用于 UI 和测试占位；其结果不得进入正式版权库、正式报告、云同步或跨端 QA 证据。
- `RustWatermarkBridge` 已包住 FRB 生成的图片与音频写入 / 提取 API，并支持提取。
...
- `CloudAccountClient` 已实现正式 `POST /v1/auth/challenges` 和 `POST /v1/auth/sessions` 的请求 / 响应模型，可把云端返回映射进 `SyncProfile`；`auth/continue` 仅作为后端兼容 alias 保留。
...
- 设置页主路径是“登录账户并同步权益 / 版权库”；LAN 地址和配对码只出现在高级区。
- 本地预览登录会生成账户契约快照，包括 `account_id`、`workspace_id`、`device_id`、`creator_profile_id` 和 `entitlement_id`；正式后端主路径对应 `auth/challenges -> auth/sessions`。
